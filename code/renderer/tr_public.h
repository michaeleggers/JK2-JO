...
typedef struct {
	...
	int		(*Font_HeightPixels)(const int index, const float scale); // scale = 1.0f
	int		(*Font_StrLenPixels)(const char *s, const int index, const float scale); // scale = 1.0f
	int		(*Font_StrLenChars) (const char *s);
	void	(*Font_DrawString)(int x, int y, const char *s, const float *rgba, const int iFontHandle, int iMaxPixelWidth, const float scale); // scale = 1.0f
